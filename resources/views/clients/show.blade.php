                                    ...
                                    <div class="bg-gradient-to-r from-indigo-50 to-white rounded-lg p-4 mb-4 border border-indigo-100 flex justify-between items-center">
                                        ...
                                        <div x-show="selected.length > 0" style="display: none;">
                                            <form action="{{ route('installments.bulk-condone') }}" method="POST">
                                                ...
                                                <button type="submit" class="text-xs bg-indigo-600 text-white px-3 py-1 rounded hover:bg-indigo-700 shadow-sm" onclick="return confirm('¿Seguro que deseas condonar los intereses de TODAS las cuotas seleccionadas?');">
                                                    Condonar (<span x-text="selected.length"></span>)
                                                </button>
                                            </form>
                                        </div>
                                    </div>
                                    ...
                                    <div class="overflow-x-auto rounded-lg border border-gray-200 shadow-sm">
                                        <table class="w-full text-sm">
                                            <thead class="text-xs text-gray-700 uppercase bg-gradient-to-r from-gray-50 to-gray-100">
                                                <tr>
                                                    <th class="px-2 py-3 w-4">
                                                        <input type="checkbox" @change="allSelected = !allSelected; selected = allSelected ? [{{ $plan->installments->pluck('id')->join(',') }}] : []" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                                    </th>
                                                    ...
                                                    <th class="px-4 py-3 text-left font-semibold">Pagado / Fechas</th>
                                            ...
                                            <tbody class="divide-y divide-gray-200 bg-white">
                                                @foreach ($plan->installments->sortBy('installment_number') as $installment)
                                                    ...
                                                    <tr class="hover:bg-indigo-50 transition-colors duration-150 group">
                                                        ...
                                                        {{-- Checkbox --}}
                                                        <td class="px-2 py-3">
                                                            <input type="checkbox" value="{{ $installment->id }}" x-model="selected" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                                                        </td>
