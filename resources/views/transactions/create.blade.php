                            ...
                            <div x-show="!loading && filteredInstallments.length > 0" class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                                <div class="max-h-96 overflow-y-auto">
                                    <table class="w-full text-sm">
                                        ...
                                        <template x-for="group in groupedByLotAndService" :key="group.lot">
                                            <template x-for="serviceGroup in group.services" :key="group.lot + '|' + serviceGroup.service">
                                                <tbody class="divide-y divide-gray-100 border-t-4 border-white">
                                                    <tr class="bg-indigo-50 border-t-2 border-indigo-200">
                                                        <td colspan="5" class="px-4 py-2">
                                                            <div class="flex flex-wrap items-center gap-2">
                                                                ...
                                                                <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[11px] font-extrabold uppercase tracking-wider bg-amber-100 text-amber-800 border border-amber-200">
                                                                    <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                                                                    <span x-text="serviceGroup.service"></span>
                                                                </span>
                                                            </div>
                                                        </td>
                                                    </tr>
                                                    <template x-for="inst in serviceGroup.installments" :key="inst.id">
                                                        <tr class="hover:bg-indigo-50 transition-colors duration-150 cursor-pointer" @click="if ($event.target.type !== 'checkbox') { $el.querySelector('input[type=checkbox]').click() }">
                                                            ...
                                                            <td class="p-4 text-gray-900 font-medium" x-text="serviceGroup.service"></td>
                                                            <td class="p-4 text-center">
    ...
    <script>
        document.addEventListener('alpine:init', () => {
            Alpine.data('paymentForm', () => ({
                ...
                get groupedByLotAndService() {
                    const lotMap = new Map();
                    (this.filteredInstallments || []).forEach(inst => {
                        const lotKey = inst.payment_plan?.lot?.identifier ?? 'Sin lote';
                        const serviceKey = inst.payment_plan?.service?.name ?? 'Sin servicio';
                        if (!lotMap.has(lotKey)) lotMap.set(lotKey, new Map());
                        const serviceMap = lotMap.get(lotKey);
                        if (!serviceMap.has(serviceKey)) serviceMap.set(serviceKey, []);
                        serviceMap.get(serviceKey).push(inst);
                    });
                    return Array.from(lotMap.entries()).map(([lot, serviceMap]) => ({
                        lot,
                        services: Array.from(serviceMap.entries()).map(([service, installments]) => ({ service, installments }))
                    }));
                }
            }));
        });
