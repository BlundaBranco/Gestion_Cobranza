                ...
                <tr class="hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-4 py-3 text-right">
                        <div class="flex items-center justify-end gap-2">
                            ...
                            @if ($installment->interest_amount > 0 && $installment->transactions->isEmpty())
                                <form action="{{ route('installments.condone', $installment) }}" method="POST" onsubmit="return confirm('¿Seguro que deseas condonar los intereses?');">
                                    @csrf
                                    <button type="submit" class="text-xs font-medium text-blue-600 hover:text-blue-800 hover:underline px-2">Condonar</button>
                                </form>
                            @endif

                            @if($remaining > 0.005)
                                @if($lot->client && $lot->client->phone)
                                    <a href="{{ generate_whatsapp_message($installment, $remaining) }}" target="_blank" class="flex items-center text-xs font-medium text-green-600 hover:text-green-800 transition-colors px-2" title="Enviar WhatsApp">
